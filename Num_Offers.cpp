...
Num_Offers::Num_Offers(const Num_Offers& orig) {
}
...
QString Num_Offers::getName() {
    return "Deep of Num offers";
}

int Num_Offers::getNoOfCharts() {
    return 3;
}

int Num_Offers::getChartsGroup() {
    return 1;
}
...
void Num_Offers::createChart(QChartViewer *viewer, const int &ticker, QDateTime startDate, QDateTime endDate, int TimeFrame, QString ChartType, int BallSize) {
...
bool Num_Offers::getData(const int &ticker, QDateTime startDate, QDateTime endDate, int TimeFrame) {
    ...
    //
    if (!db.isOpen()) {
        db = QSqlDatabase::addDatabase("QPSQL", "settingquotes_num_offers_conn");
    ...
    for (int i = 0; i < model0->rowCount(); i++) {
        ...
        m_highData[i] = rec.value("offerdepththigh").toDouble();
        m_lowData[i] = rec.value("offerdepthtlow").toDouble();
        m_openData[i] = rec.value("offerdepthtopen").toDouble();
        m_closeData[i] = rec.value("offerdepthtclose").toDouble();
