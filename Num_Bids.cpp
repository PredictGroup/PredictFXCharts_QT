...
Num_Bids::Num_Bids(const Num_Bids& orig) {
}
...
QString Num_Bids::getName() {
    return "Deep of Num bids";
}

int Num_Bids::getNoOfCharts() {
    return 2;
}

int Num_Bids::getChartsGroup() {
    return 1;
}
...
void Num_Bids::createChart(QChartViewer *viewer, const int &ticker, QDateTime startDate, QDateTime endDate, int TimeFrame, QString ChartType, int BallSize) {
...
bool Num_Bids::getData(const int &ticker, QDateTime startDate, QDateTime endDate, int TimeFrame) {
    ...
    //
    if (!db.isOpen()) {
        db = QSqlDatabase::addDatabase("QPSQL", "settingquotes_num_bids_conn");
    ...
    for (int i = 0; i < model0->rowCount(); i++) {
        ...
        m_highData[i] = rec.value("biddepththigh").toDouble();
        m_lowData[i] = rec.value("biddepthtlow").toDouble();
        m_openData[i] = rec.value("biddepthtopen").toDouble();
        m_closeData[i] = rec.value("biddepthtclose").toDouble();
